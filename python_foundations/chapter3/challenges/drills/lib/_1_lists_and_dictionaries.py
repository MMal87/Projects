...
def starts_with_a(lst):
    return [i for i in lst if i[0] == 'a']
...
def starts_with_a_vowel(lst):
    return [i for i in lst if i[0] in ('a', 'e', 'i', 'o', 'u')]
...
def where_value_below(dict, num):
    new_dict = {key:value for key, value in dict.items() if value < num}
    return new_dict
